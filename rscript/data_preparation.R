...
issued_forecast_original <- read_excel("data/full_issued_data.xlsx") |> janitor::clean_names() 

# full_issued <- read_excel("data/full_issued_data.xlsx") |> janitor::clean_names()

write_rds(issued_forecast_original,"data/full_issued.rds") 

full_issues_original <- read_rds("data/full_issued.rds")

stock_out_days_original <- read_excel("data/number_of_stock_out_days.xlsx") |> janitor::clean_names()

full_issues_original <- full_issues_original |> filter(item  !="Rifampicin + Isoniazid + Pyrazinamide + Ethambutol (RHZE)")

HP_wastage_rate_original <- read_excel("data/stock_out_data.xlsx", sheet = "HP wastage rate")|>
  janitor::clean_names()
...
#--------monthly time series
full_issues_correct_amountissues <- read_rds("data/full_issues_correct_amountissues.rds")

issues_selected <- full_issues_correct_amountissues |> select(transaction_date,item,quantity_product_issued,amount_issued)
...
daily_issues_tsibble <- daily_issues |> as_tsibble(index = date, key=item) |> 
 fill_gaps(quantity_product_issued = 0, amount_issued=0,.full = TRUE)
...
# monthly_issues_tsibble1 <- daily_issues_tsibble1 |> index_by(month=yearmonth(date)) |> group_by_key() |> 
#   summarise(quantity_product_issued=sum(quantity_product_issued),
#             amount_issued=sum(amount_issued))
