...
snaive <- read_rds('results/snaive_prob.rds') |> 
  ...
  group_by(month, item_id) |> 
  mutate(horizon = row_number()) |> # creating the horizon
  ungroup() |> 
  mutate(unique_id = paste0(item_id,'-',horizon))
...
pred_master <- read_rds('results/timegpt_fc.rds') |> 
  ...
  rename(origin = horizon) |> # i have mistakenly named origin as horizon
  group_by(month, item_id, model) |> 
  mutate(horizon = row_number()) |> # creating the horizon
  ungroup() |> 
  mutate(unique_id = paste0(item_id,'-',horizon))

point_pred <- pred_master |> 
  select(month, item_id, horizon, model, prediction) |> 
  mutate(unique_id = paste0(item_id,'-',horizon))
...
fc_accuracy <- fc_accuracy |> 
  separate(id, into = c("item_id", "horizon"), sep = "-") |> 
...
write_rds(fc_accuracy, 'results/point_accuracy_horizon.rds')
...
bs_crps_summary <- bs_crps |> 
  ...
  summarise(mean_crps = mean(crps),
            mean_winkler_score = mean(winkler_score))

write_rds(bs_crps, 'results/prob_accuracy_horizon.rds')
